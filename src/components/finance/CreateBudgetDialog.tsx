...
export const CreateBudgetDialog: React.FC<CreateBudgetDialogProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await logInfo('Budget created successfully', {
        family_id: currentFamily.id,
        budget_name: formData.name
      }, 'finance', 'budget-creation');

      toast.success({ 
        title: t('finance.budgetCreatedSuccessfully'),
        description: t('finance.budgetCreatedDescription', { name: formData.name })
      });
      ...
      await logError('Failed to create budget', error, {
        family_id: currentFamily?.id,
        form_data: formData
      }, 'finance', 'budget-creation');

      toast.apiError(error, 'creating budget');
    } finally {
      setLoading(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={(isOpen) => {
      ...
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {t('finance.createBudgetDescription')}
          </DialogDescription>
        </DialogHeader>
