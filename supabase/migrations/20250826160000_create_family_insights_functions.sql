-- Family Insights and Multi-Member Spending Analysis Functions

-- Function to get family member spending patterns
...
-- Function to get shared vs individual expense analysis
...
-- Function to get family spending comparison and rankings
...
DECLARE
  ...
  family_total_transactions INTEGER;
BEGIN
  ...
  WITH expense_ranking AS (
    SELECT 
      ...
      ROW_NUMBER() OVER (ORDER BY COALESCE(SUM(CASE WHEN t.type = 'expense' THEN t.amount END), 0) DESC)::INTEGER as rank_position
    FROM family_members fm
    JOIN profiles p ON p.id = fm.user_id
    LEFT JOIN transactions t ON t.created_by = fm.user_id 
      AND t.date >= start_date_param 
  ...
  transaction_count_ranking AS (
    SELECT 
      ...
      ROW_NUMBER() OVER (ORDER BY COUNT(t.id) DESC)::INTEGER as rank_position
    FROM family_members fm
    JOIN profiles p ON p.id = fm.user_id
    LEFT JOIN transactions t ON t.created_by = fm.user_id 
      AND t.date >= start_date_param 
  ...
  avg_transaction_ranking AS (
    SELECT 
      ...
      ROW_NUMBER() OVER (ORDER BY COALESCE(AVG(t.amount), 0) DESC)::INTEGER as rank_position
    FROM family_members fm
    JOIN profiles p ON p.id = fm.user_id
    LEFT JOIN transactions t ON t.created_by = fm.user_id 
      AND t.date >= start_date_param 
  ...
  SELECT 
    ...
    er.rank_position,
    CASE 
      WHEN family_total_expenses > 0 THEN ROUND((er.value / family_total_expenses * 100)::DECIMAL, 2)
      ELSE 0 
    END as percentage_of_family_total
  ...
  SELECT 
    ...
    tcr.rank_position,
    CASE 
      WHEN family_total_transactions > 0 THEN ROUND((tcr.value / family_total_transactions * 100)::DECIMAL, 2)
      ELSE 0 
    END as percentage_of_family_total
  ...
  SELECT 
    ...
    atr.value,
    atr.rank_position,
    0 as percentage_of_family_total -- Percentage doesn't make sense for averages
  FROM avg_transaction_ranking atr;
END;
$$ LANGUAGE plpgsql;

-- Function to get family financial health insights
...
BEGIN
  ...
  SELECT 
    'savings_rate' as insight_type,
    'Family Savings Rate' as insight_title,
    CASE 
    ...
    CASE 
      WHEN fm.total_income > 0 THEN
        CASE 
          WHEN (fm.total_income - fm.total_expenses) / fm.total_income < 0 THEN 'high'
          WHEN (fm.total_income - fm.total_expenses) / fm.total_income < 0.10 THEN 'medium'
          ELSE 'low'
        END
      ELSE 'high'
    END as impact_level,
  ...
  SELECT 
    'spending_inequality' as insight_type,
    'Family Spending Balance' as insight_title,
    CASE 
      WHEN si.avg_member_expenses > 0 AND si.expense_std_dev / si.avg_member_expenses > 0.5 THEN
        'High spending variation detected between family members. Consider discussing budget allocations.'
      WHEN si.avg_member_expenses > 0 AND si.expense_std_dev / si.avg_member_expenses > 0.3 THEN
        'Moderate spending variation between family members. This is normal but worth monitoring.'
      ELSE
        'Family spending is well-balanced across members.'
    END as insight_description,
    CASE 
      WHEN si.avg_member_expenses > 0 AND si.expense_std_dev / si.avg_member_expenses > 0.5 THEN 'medium'
      ELSE 'low'
    END as impact_level,
    CASE 
      WHEN si.avg_member_expenses > 0 AND si.expense_std_dev / si.avg_member_expenses > 0.5 THEN
        'Hold a family meeting to discuss spending goals and set individual budgets for discretionary expenses.'
      ELSE
        'Continue current spending patterns and regular family financial check-ins.'
    END as recommended_action,
    jsonb_build_object(
      'avg_member_expenses', si.avg_member_expenses,
      'max_member_expenses', si.max_member_expenses,
      'min_member_expenses', si.min_member_expenses,
      'coefficient_of_variation', 
        CASE WHEN si.avg_member_expenses > 0 THEN 
          ROUND((si.expense_std_dev / si.avg_member_expenses)::DECIMAL, 3)
        ELSE 0 END
    ) as supporting_data
  FROM spending_inequality si;
END;
$$ LANGUAGE plpgsql;