-- Fix ambiguous column references in budget recommendations
-- The issue is that 'recommended_budget' appears in multiple CTEs without proper qualification
...
BEGIN
  ...
  total_family_expenses AS (
    SELECT COALESCE(SUM(ca.total_spending), 0) as total_expenses
  ...
  category_recommendations AS (
    SELECT 
      ...
      CASE 
        WHEN ca.total_spending > (SELECT tfe.total_expenses * 0.15 FROM total_family_expenses tfe) THEN
      ...
      END as category_recommended_budget,
      ...
      CASE
        ...
        ELSE 0.60
      END as category_confidence_score
    FROM category_analysis ca
  ),
  category_potential_savings AS (
    SELECT 
      cr.category_name,
      cr.total_spending,
      cr.category_recommended_budget,
      cr.category_confidence_score,
      (cr.total_spending - cr.category_recommended_budget) as category_potential_savings,
      cr.spending_variance
    FROM category_recommendations cr
    WHERE (cr.total_spending - cr.category_recommended_budget) > 50
  )
  -- High-impact reduction recommendations
  SELECT 
    'reduce_category'::VARCHAR as recommendation_type,
    cps.category_name::VARCHAR,
    cps.total_spending,
    cps.category_recommended_budget as recommended_budget,
    cps.category_potential_savings as potential_savings,
    cps.category_confidence_score as confidence_score,
    CASE 
      WHEN cps.total_spending > (SELECT tfe.total_expenses * 0.20 FROM total_family_expenses tfe) THEN
      ...
      WHEN COALESCE(cps.spending_variance, 0) > (SELECT AVG(COALESCE(cr2.spending_variance, 0)) FROM category_recommendations cr2) THEN
    ...
    END as reason,
    jsonb_build_object(
      'set_monthly_limit', ROUND((cps.category_recommended_budget * monthly_multiplier)::DECIMAL, 2),
      'track_daily', true,
      'review_frequency', 'weekly',
      'suggest_alternatives', CASE WHEN cps.category_name IN ('Dining', 'Entertainment', 'Food', 'Restaurant') THEN true ELSE false END
    ) as action_items
  FROM category_potential_savings cps
  ...
  SELECT 
    ...
    GREATEST(
      ...
      COALESCE((SELECT SUM(cps2.category_potential_savings) FROM category_potential_savings cps2), 0), 
      0
    ...
    jsonb_build_object(
      'current_savings_rate', CASE 
        WHEN fi.total_monthly_income > 0 THEN 
          ROUND(((fi.total_monthly_income - COALESCE((SELECT SUM(ca.total_spending) * monthly_multiplier FROM category_analysis ca), 0)) / fi.total_monthly_income * 100)::DECIMAL, 1)
        ELSE 0 
      END,
